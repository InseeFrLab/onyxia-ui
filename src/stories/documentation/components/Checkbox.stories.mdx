...
This is a demo of `<Checkbox/>` component.
...
- [ternary buttons](#ternary-buttons)
...
<Canvas>
	<Story name="Buttons with right icon">
		{[false, true].map(dark=>
			<Fragment key={dark}>
				<StoryProvider dark={dark}>
...
<Canvas>
	<Story name="Buttons with left icon">
		{[false, true].map(dark=>
			<Fragment key={dark}>
				<StoryProvider dark={dark}>
